...
namespace BcBroker.UserControls
{
    ...
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(BrokerControl));
                ...
                if (savedFile.Exists)
                {
                    using (StreamReader a = savedFile.OpenText())
                    {
                        ...
                        if (con != null)
                        {
                            ...
                            numTotalBuyAcc.Value = con.TotalAccBuy;
                            dgvAccountBuy.Rows.Clear();
                            dgvAccountSell.Rows.Clear();
                            ...
                            foreach (var item in con.SellAccounts)
                            {
                                dgvAccountSell.Rows.Add(item.serverType, item.username, item.password, item.rate, "Login", "", "", "", item.domain, "", item.IsChecked, item.IP);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var config = new ConfigModel
                {
                    ...
                    TotalAccBuy =   (int)numTotalBuyAcc.Value,
                ...
                foreach (DataGridViewRow item in dgvAccountSell.Rows)
                {
                    if (item.Cells[1].Value != null && item.Cells[2].Value != null
                        && !String.IsNullOrEmpty(item.Cells[1].Value.ToString())
                        && !String.IsNullOrEmpty(item.Cells[2].Value.ToString())
                        )
                    {
                        ...
                        config.SellAccounts.Add(new Account
                        {
                            serverType = item.Cells[0].Value.ToString(),
                            username = item.Cells[1].Value.ToString(),
                            password = item.Cells[2].Value.ToString(),
        ...
        public void StartProcessBroker()
        {
                        ...
                        && broker.IbetEn.AccountStatus == eAccountStatus.Online)
            {
                ...
                broker.TotalAccSell= (int)numTotalBuyAcc.Value;
