...
namespace BcWin.Core.WCF
{
    ...
    {
        public static IBcService InitService(string endpointAddress)
        {
            BasicHttpBinding bindding = new BasicHttpBinding();
            bindding.ReceiveTimeout = TimeSpan.FromMinutes(10);
            bindding.SendTimeout = TimeSpan.FromMinutes(10);
            bindding.MaxReceivedMessageSize = 2147483647;
            //bindding.ReaderQuotas.MaxArrayLength = 2147483647;
            //bindding.ReaderQuotas.MaxBytesPerRead = 2147483647;
            ChannelFactory<IBcService> factory = new ChannelFactory<IBcService>(bindding, endpointAddress);
            IBcService proxy = factory.CreateChannel();
